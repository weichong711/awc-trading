...
export function AdminPanel({ accessToken }: AdminPanelProps) {
  ...
  const stats = {
    total: users.length,
    trial: users.filter((u) => u.access.status === "trial")
      .length,
    active: users.filter((u) => u.access.status === "active")
      .length,
    grace: users.filter(
      (u) => u.access.status === "grace_period",
    ).length,
    blocked: users.filter((u) => u.access.status === "blocked")
      .length,
  };
  ...
  const handleBlock = async () => {
    ...
    try {
      const res = await fetch(
        `${SERVER}/admin/users/${actionUser.id}/block`,
        {
          method: "POST",
          headers: {
  ...
  const handleUnblock = async () => {
    ...
    try {
      const res = await fetch(
        `${SERVER}/admin/users/${actionUser.id}/unblock`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            className="pl-9"
            placeholder="Search by email, business name, username..."
            value={search}
                        ...
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2.5">
                              <div
                                className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${isBlocked ? "bg-red-100 text-red-600" : "bg-primary/10 text-primary"}`}
                              >
                                {user.email
                                  .charAt(0)
                                  .toUpperCase()}
                              ...
                              <div>
                                <p className="font-medium text-xs leading-tight truncate max-w-[140px]">
                                  {user.email}
                                </p>
                                <p className="text-xs text-muted-foreground">
                                  {user.username || "—"}
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              This will immediately lock the user out. They will
              see a "Subscription Suspended" screen.
            </DialogDescription>
          </DialogHeader>
          {actionUser && (
            <div className="space-y-4">
              <div className="p-3.5 bg-red-50 border border-red-200 rounded-xl">
                ...
                <p className="text-xs text-muted-foreground">
                  {actionUser.businessName}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Current status:{" "}
                  <span className="font-medium capitalize">
                    {actionUser.access.status.replace("_", " ")}
                  </span>
                </p>
              </div>
              <div>
              ...
              return (
                <div className="space-y-4 text-sm">
                  ...
                  {[
                    {
                      label: "User ID",
                      value: actionUser.id.slice(0, 16) + "...",
                    },
                    {
                      label: "Username",
                    ...
                    {
                      label: "Registered",
                      value: fmtDatetime(actionUser.createdAt),
                    },
                    {
                      label: "Last Login",
                      value: fmtDatetime(actionUser.lastSignIn),
                    },
                    {
                      label: "Plan",
                      value: (
                        actionUser.subscription?.plan || "—"
                      ).toUpperCase(),
                    },
                    {
                      label: "Expires",
                      value: fmtDate(
                        actionUser.subscription?.expiresAt,
                      ),
                    },
                    {
                      label: "Grace End",
                      value: fmtDate(
                        actionUser.subscription
                          ?.gracePeriodEndsAt,
                      ),
                    },
                    {
                    ...
                    >
                      ...
                      <span
                        className={`font-medium text-xs ${label === "Manual Block" && value === "YES" ? "text-red-600" : ""}`}
                      >
                        {value}
                      </span>
                    </div>
                  ...
                  {/* Payment history */}
                  {actionUser.subscription?.paymentHistory &&
                    actionUser.subscription.paymentHistory
                      .length > 0 && (
                      <div>
                        ...
                        <div className="space-y-1.5 max-h-32 overflow-y-auto">
                          {actionUser.subscription.paymentHistory.map(
                            (p, i) => (
                              <div
                                key={i}
